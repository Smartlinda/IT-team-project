...
<script >
	  ...
	  function getRoundCounter() {
				...
				var xhr = createCORSRequest('GET', "//localhost:7777/toptrumps/getRoundCounter"); // Request type and URL
				...
				xhr.onload = function(e) {
					...
					document.getElementById("round").innerHTML = JSON.stringify(responseText);
			...
			function checkForWinner() {
				...
				// First create a CORS request, this is the message we are going to send (a get request in this case)
				var xhr = createCORSRequest('GET', "//localhost:7777/toptrumps/game/checkForWinner"); // Request type and URL
				...
				xhr.onload = function(e) {
					...
					//alert(responseText); // lets produce an alert
					document.getElementById("winner").innerHTML = JSON.stringify(responseText);
            ...
            <h4>Round:</h4> 
            <p id="round" style="color: white">"0000000000000000000000000000000000000000"</p>
	        <h4>The active player is:</h4> 
	        <p id="activeplayer" style="color: white">"0000000000000000000000000000000000000000"</p>
	       
	        <p id="HumaSelectCategoryvar" style="color: white">"0000000000000000000000000000000000000000"</p>
		
	       	
	 		
			<form accept-charset="utf-8">
			<p>Please select a category and click on Attack!</p>
              <select id="choose_category"   onchange="DisplayChosenCategory();"    class="dropdown1" name="Number" style="left:90%;position:relative">
              <option id="Health" value="Health">Health</option>
              <option id="Stamina" value="Stamina">Stamina</option>
              <option id="Attack" value="Attack">Attack</option>
              <option id="Armor" value="Armor">Armor</option>
              <option id="Size" value="Size">Size</option>
              
              </select>
              <input     id="choose_category" class="w3-button w3-white"  onclick="return  PopUp() " style="top:60%; left:100%; " value="Attack!">
               
           </form>
	 		
	        <h4>The selected category is:</h4> 
	        <p id="SelectedValueChosenCategory" style="color: white"></p>
	        <h4>The winner is:</h4> 
	        <p id="winner" style="color: white">"0000000000000000000000000000000000000000"</p>
	
	<script>
	
	function DisplayChosenCategory() {
	
	var selectedValue = document.getElementById("choose_category").value;
	document.getElementById("SelectedValueChosenCategory").innerHTML=selectedValue;
	...
	alert("Distribute cards again");
	
	}
	
	
	
		function humanSelectCategory() {
		
			var xhr = createCORSReq('GET', "http://localhost:7777/toptrumps/humanSelectCategory");
			
			xhr.onload = function(e) {
			var responseText = xhr.response; 
				
				
			document.getElementById("HumaSelectCategoryvar").innerHTML = JSON.stringify(responseText);
			//document.getElementById("choose_category").innerHTML = JSON.stringify(responseText);
	...
	<div class="container">
		...
		function getPlayerState() {
			...
			xhr.onload = function(e) {
				var responseText = xhr.response;
				//alert(responseText);
				document.getElementById("activeplayer").innerHTML = JSON.stringify(responseText);
			}
		}
		...
		// Set the values of each card in the iframe
